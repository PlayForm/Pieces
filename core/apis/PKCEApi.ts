...
export class PKCEApi extends runtime.BaseAPI {
	 ...
	 * /pkce/code [POST]
	 */
	async generateCodeRaw(
		requestParameters: GenerateCodeRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<PKCE>> {
		const queryParameters: any = {};
	...
	async generateCode(
		...
		const response = await this.generateCodeRaw(
			requestParameters,
			initOverrides,
		);
	...
	async generateToken(
		...
		const response = await this.generateTokenRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
	 ...
	 */
	async respondWithCodeRaw(
		requestParameters: RespondWithCodeRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<PKCE>> {
		if (
			...
			throw new runtime.RequiredError(
				"code",
				"Required parameter requestParameters.code was null or undefined when calling respondWithCode.",
			);
		}
		...
		if (
			...
			throw new runtime.RequiredError(
				"state",
				"Required parameter requestParameters.state was null or undefined when calling respondWithCode.",
			);
		}
	...
	async respondWithCode(
		...
		const response = await this.respondWithCodeRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
